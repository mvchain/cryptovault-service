...
public interface AppProjectMapper extends MyMapper<AppProject> {
    ...
    @Update("update app_project set `status` = 2, updated_at = #{currentTimeMillis} WHERE id = #{id} and updated_at = #{updatedAt}")
    Integer updateProjectStopStatus(@Param("id") BigInteger id, @Param("updatedAt") Long updatedAt,  @Param("currentTimeMillis") Long currentTimeMillis);
